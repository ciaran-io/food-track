...
available_options = ["Add new food entry", "View today's foods",
                     "Set weekly calorie target",
                     "View today's calories", "View weekly calories", "Menu",
                     "Quit"]
...
for option in range(len(available_options)):
    ...
    if option == 5:
        ...
        food_options_table.add_row(
            [str(option + 1), available_options[option]]
        )
...
food_table.field_names = ["Food", "Calories", "Weight(g)", "Target",
                          "Remaining", "Percentage"]

# Create a weekly calorie table
weekly_food_table = PrettyTable()
weekly_food_table.field_names = ["Day", "Calories", "Target", "Remaining",
                                 "Percentage"]
...
remaining_calories_table.field_names = ["Calories", "Target", "Remaining",
                                        "Percentage"]
...
def create_daily_food_table(food_data, foods_sheet, target):
    ...
    for food in food_data:
        ...
        food_table.add_row(
            [foods_sheet.cell(row, 2).value, foods_sheet.cell(row, 3).value,
             foods_sheet.cell(row, 4).value, target, remaining_calories,
             percentage])
...
def create_weekly_food_table(weekly_calories, target):
    ...
    for day in range(len(weekly_calories)):
        remaining_calories = return_remaining_calories(weekly_calories[day],
                                                       target)
        percentage = return_percentage(remaining_calories, target)

        # Add the data to the table
        weekly_food_table.add_row(
            [days[day], weekly_calories[day], target, remaining_calories,
             percentage]
        )
